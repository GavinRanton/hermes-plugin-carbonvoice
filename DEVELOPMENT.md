...
| **Forwarded messages** | A forward carries `share_link_id` (alias: deprecated `forward_id`) on the wrapper message; the original content lives behind `GET /v3/message-sharelinks/:id` (`shared_message`). The adapter composes `[Forwarded message from <name>]` + original transcript + attachment lines, then `[Forwarded by <user>]` + the comment. Forwarded **images** are downloaded via the share-link-scoped signed-URL route (`GET /message-sharelinks/:sl/attachments/signedurl/:att` — the link authorizes, no channel access needed) and ride `media_urls`; links go inline; other types are noted but not imported. Share-link fetch/download failures reuse the stuck-message hold (retry, then placeholder past `CARBONVOICE_STUCK_MAX_AGE_S`). Mirrors cv-claude-channels. |
...
| **No native "list messages in thread" endpoint on cv-api.** PR 4's thread-context fetch combines two REST calls (`GET /messages/<channel_id>/index` for ids + `parent_message_id`, then `POST /v6/messages/by-ids` for transcripts) and filters client-side. Works, but pays the index fetch on every cold cache. | One extra index call per thread per TTL window. Negligible for low-volume workspaces; worth optimizing for high-volume ones. | Backend ask: `GET /v5/messages/<thread_id>/replies?limit=N` or extend `getMessageIdsV5` to accept `thread_id` filter. Collapses the workaround to a single call. |
...
| ~~**Voice-tag mention lost to a read race (bot never answered a tagged voice memo).**~~ ✅ **Resolved (2026-06-12).** Flutter applies picker tags via the batch `PUT /messages/:id/tagged-users` only after STT (~10–30s post-create). cv-api *does* bump `last_updated_at` and emit `message:updated` on that PUT (`tag-user.service.ts broadcastTagChange`) — but the plugin could still lose the mention two ways: (1) the tick triggered by the tag event could fetch within ~100ms of the write and read a stale copy (no tags) → revisitable gate reject → `False` → cursor advanced past the message, and since the tag event is the LAST update that message ever emits, it was dead forever; (2) the old tick coalescing **dropped** a tick that arrived while a fetch was in flight — if that dropped tick was the tag event, same outcome. Observed live: tag landed at +30s, cursor ended 121ms past the message's `updated_at`, no ack, no dispatch. Fix, three layers: **(a)** revisitable gate rejections of *voice* messages younger than `CARBONVOICE_REVISIT_MAX_AGE_S` (default 90s) now hold the cursor (return `None`, same machinery as stuck transcripts) instead of advancing — text messages carry tags on the create body and never hold; **(b)** tick coalescing now queues a *trailing* re-fetch instead of dropping (`_tick_pending`); **(c)** while anything is held, a one-shot self-scheduled re-tick (6s) keeps retries flowing in WS mode (polling stopped) without depending on the next unrelated socket event — this also benefits transcript-stuck messages on quiet workspaces. Plus a staleness guard in the v5 enrichment: if the v2 push payload has `tagged_user_ids` and the v5 GET copy doesn't, keep the populated array. | Tagged voice memos in groups now reach the bot reliably, even when the tag write races the re-fetch. | — |
| **Forwarded voice comments can race STT.** A forward whose *comment* is a voice memo (wrapper `is_text_message: false`) waits for STT like any voice message, but past `CARBONVOICE_STUCK_MAX_AGE_S` the forward is processed with the original content only — the never-transcribed comment is dropped rather than losing the whole forward. Text comments and comment-less forwards are unaffected (they bypass the transcript wait entirely; the wrapper of a comment-less forward never gets a transcript of its own). | Rare: only voice comments whose STT exceeds the stuck window (default 5 min) lose the comment. | None planned — the trade-off mirrors cv-claude-channels (which processes forwards regardless of wrapper transcript state). |
...
| ~~**Message reads on v3/v5 endpoints.**~~ ✅ **Resolved (Sep 2026 — cv-api v6 read migration).** All message *reads* use cv-api `/v6`: the poll fallback pages `GET /v6/messages/updates` (`direction=newer`, `date` on the first page then `cursor=next_cursor` while `has_more`; filter is `conversation_id`, legacy `channel_id` is a 400) and persists the tail `next_cursor` in `$HERMES_HOME/state/carbonvoice.json` next to the `lastSeenAt` date seed; single / batch / share-link reads use `GET /v6/messages/:id`, `POST /v6/messages/by-ids`, `GET /v6/message-sharelinks/:id`. MessageV6 is mapped back to the legacy shape once at the fetch boundary (`parse.normalize_v6`) because Socket.IO still pushes the legacy shape through the same parse path. Re-deliveries (~4s window) are dropped by `id` + `last_updated_at`; a 400 on a cursor re-anchors by date, any other failure keeps it; the cursor is persisted only after the whole batch was handled. `GET /messages/:channel_id/index` has no v6 equivalent and stays. Sends are unchanged. | Poll + enrichment on the current contract; no more timestamp-window re-fetch on every tick. | — |
